...
InputSocket::InputSocket(rclcpp::Node* private_nh, uint16_t port) : Input(private_nh, port) {
    ...
    // should be AF_INET, technically UDP is protocol number 17 but 0 should be ok too
    ...
    if (setsockopt(sockfd_, SOL_SOCKET, SO_REUSEADDR, (const void*)&opt, sizeof(opt))) {
        // probably would want to do errno() here to get the actual error that was thrown
    ...
    // FIXME: we shouldnt need to use FASYNC
...
int InputSocket::getPacket(lslidar_msgs::msg::LslidarPacket::UniquePtr& packet) {
    ...
    fds[0].fd = sockfd_;
    fds[0].events = POLLIN;
    static const int POLL_TIMEOUT = 2000; // 2 seconds

    sockaddr_in sender_address {};
    socklen_t sender_address_len = sizeof(sender_address);

    // FIXME: ok what is this flag thing here??
    while (flag == 1) {
        ...
        ssize_t nbytes = recvfrom(
            sockfd_, &packet->data[0], packet_size_input, 0, (sockaddr*)&sender_address, &sender_address_len);
        ...
        if (nbytes < 0) {
            ...
            if (devip_str_ != "" && sender_address.sin_addr.s_addr != devip_.s_addr) continue;
            else break; // done
        }
    }
    if (flag == 0) {
...
InputPCAP::InputPCAP(rclcpp::Node* private_nh, uint16_t port, double packet_rate, std::string filename)
    ...
    if (read_once_) RCLCPP_WARN(private_nh_->get_logger(), "Read input file only once.");
    if (read_fast_) RCLCPP_WARN(private_nh_->get_logger(), "Read input file as quickly as possible.");
...
int InputPCAP::getPacket(lslidar_msgs::msg::LslidarPacket::UniquePtr& pkt) {
    ...
    while (flag == 1) {
        ...
        if ((res = pcap_next_ex(pcap_, &header, &pkt_data)) >= 0) {
            ...
            if (!devip_str_.empty() && (0 == pcap_offline_filter(&pcap_packet_filter_, header, pkt_data))) continue;
